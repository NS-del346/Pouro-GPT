...
const usage = `Usage:
  npm --prefix tools/stitch-audit run stitch:audit -- <stitch-export.zip> [options]

Options:
  --set <set-a|set-b|set-c|all>  Screen/rule set (default: set-a)
  --output <directory>           Output directory (default: artifacts/stitch-audit)
  --strict                       Enforce hidden-file and identical-state strict checks
...
async function main() {
    ...
    try {
        ...
        if (options.jsonOnly) {
            ...
            process.stderr.write(`Stitch audit: BLOCKED\n${error instanceof Error ? error.message : String(error)}\nReport: ${report.artifacts.reportMarkdown}\n`);
        }
        process.exitCode = 2;
    }
}
