export function perceptualSimilarity(left, right) {
...
export function detectDuplicates(candidates, nearThreshold, strict) {
    const duplicates = [];
    const findings = [];
    for (let leftIndex = 0; leftIndex < candidates.length; leftIndex += 1) {
        for (let rightIndex = leftIndex + 1; rightIndex < candidates.length; rightIndex += 1) {
            ...
            const distinctRequiredStates = left.required && right.required && left.screenId !== right.screenId;
            const visuallyIdentical = exact || similarityScore === 1;
            findings.push({
                screenId: left.screenId,
                severity: strict && distinctRequiredStates && visuallyIdentical ? "error" : "warning",
